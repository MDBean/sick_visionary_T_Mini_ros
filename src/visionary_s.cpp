...
image_transport::Publisher gPubZ, gPubConfidence, gPubRGBA;
...
void publishConfidence(std_msgs::Header header, VisionarySData& dataHandler)
{
...
void publishRGBA(std_msgs::Header header, VisionarySData& dataHandler)
{
    ...
    sensor_msgs::ImagePtr msg = cv_bridge::CvImage(std_msgs::Header(), sensor_msgs::image_encodings::RGBA8, m).toImageMsg();
...
void publish_frame(VisionarySData& dataHandler) {
    ...
    if (gPubRGBA.getNumSubscribers() > 0)
    {
        publishedAnything = true;
...
int main(int argc, char **argv) {
    ...
    gPubZ                 = it.advertise("z", 1, (image_transport::SubscriberStatusCallback)on_new_subscriber_it, image_transport::SubscriberStatusCallback());
    ...
    gPubRGBA              = it.advertise("rgba", 1, (image_transport::SubscriberStatusCallback)on_new_subscriber_it, image_transport::SubscriberStatusCallback());
